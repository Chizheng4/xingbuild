...
test("v0.26.2 keeps the approved Baseline 1 to 2 shared composition contract", () => {
...
test("v0.26.2 closes optional labels and preserves the registered release action", () => {
  ...
  assert.match(practice, /practice\.boundary \? <p className="product-hero__boundary">/);
  assert.match(showcase, /const label = typeof module\.group === "string"/);
  ...
  assert.match(closing, /const eyebrow = closing\.eyebrow \?\? "继续进入"/);
  assert.match(article, /headingLevel = 2/);
  assert.match(article, /const Heading = `h\$\{headingLevel\}`/);
});

test("v0.26.2 keeps media-only elevation, muted text and mobile action geometry", () => {
