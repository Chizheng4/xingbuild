...
test("v0.26.5 preserves H-02 inside the independent Home product projection", () => {
  ...
  assert.match(home, /align="start"/);
  assert.match(practice, /align = "center"/);
  assert.match(practice, /product-hero--start/);
});

test("v0.26.5 H-02 keeps the shared four-pixel binding and removes narrow-screen padding", () => {
  ...
  assert.match(pages, /\.home-product-section \.product-hero--start \{ padding-block-start: 0; \}/);
