...
- X 平台信息只有在保留原始帖子 URL、账号身份和发布时间后才可进入候选。公司或负责人原帖属于 company statement，不自动成为独立 verified fact；涉及运营规模、安全、监管、收入等重要事实时应优先以监管、公司公告或可信媒体交叉验证。截图、搬运号和匿名爆料不能作为唯一发布来源。
...
- 日常最小终端交接目标为：审核确认后执行 `npm run content:approve -- --slug <slug> --authority <authority>`，再创建独立内容提交，并执行 `./publish-content.command --slug <slug>`。前一命令须由 Engineering 实现为既有 review + promote 的受控聚合；提交、推送、部署和公网验收仍须作为独立状态报告。命令缺失或失败时不得绕过审核、自动发布或改用宽范围命令。
