...
- 自动化、cron、scheduled task 和其他周期性唤起资源属于 task/resource 创建；普通执行、版本推进、内容审核、发布或“继续工作”不自动包含创建、复制、更新、暂停或删除权限。已存在的调度资源必须先按其唯一登记身份复用。
- 经营观察唯一调度身份、owner、运行 task 与内容接收 task 只以 `docs/operations/经营观察信息源与覆盖合同.md` 为准；每次自动化运行产生的 task 是一次性执行记录，不得升级为新的 scheduler。内容 task 不得创建或管理经营观察调度。
