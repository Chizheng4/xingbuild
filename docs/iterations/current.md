...
修正 v0.24.30 站点快照读取器的生命周期事实源：以 content-release.json 判定 active release，以 dist manifest 做产物身份校验，避免误判为空并再次覆盖线上内容。

## 正式设计与父版本

- 正式设计：`docs/design/v0.24.31 内容生命周期事实源读取修正方案.md`。
- 继承设计：`docs/design/v0.24.30 统一站点发布快照与内容保留方案.md`。
...
- 父版本：`v0.24.30` / `75ae5de`；既有 tag/history 不修改。
...
- 不修改上游事实、已发布 v0.24.30 或内容正文/来源/status/publishedAt。
