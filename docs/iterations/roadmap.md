...
**进入条件**：用户已确认本路线图和 `v0.20.0` 方案；本轮文档整理已完成；产品 task 更新 `current.md` 后，Engineering 才可接收完整 handoff 并开始实现。
...
- 当前公开基线：`v0.19.0`。
- 当前实施版本：`v0.20.0`，唯一正式指针见 `docs/iterations/current.md`。
- `v0.21.0` 及之后仍是候选，不得提前进入实现。
