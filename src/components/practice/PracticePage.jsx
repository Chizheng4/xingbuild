...
export function ProductHero({ practice, headingLevel = 1, headingId, actions = [] }) {
  ...
  return (
    <header className={`product-hero${headingLevel > 1 ? " product-hero--compact" : ""}`}>
      ...
      <p className="product-hero__boundary">{practice.boundary}</p>
      <ActionGroup actions={actions} />
    </header>
  );
}

export function PracticeHeader({ practice, headingLevel = 1, headingId, showLatestUpdate = false, actions = robotaxiProductConfiguration.heroActions }) {
  return (
    <>
      {showLatestUpdate ? <LatestUpdateCard /> : null}
      <ProductHero practice={practice} headingLevel={headingLevel} headingId={headingId} actions={actions} />
    </>
  );
}
...
export function PracticePresentation({ practice, headingLevel = 1, headingId, showLatestUpdate = false, showClosing = false, actions = robotaxiProductConfiguration.heroActions }) {
  if (!practice) {
  ...
  return (
    <div className="practice-presentation">
      <PracticeHeader practice={practice} headingLevel={headingLevel} headingId={headingId} showLatestUpdate={showLatestUpdate} actions={actions} />
